...
function bindEvents() {
  ...
  el.exportCsvButton.addEventListener("click", handleExportCsv);
...
function handleExportCsv() {
  if (!state.expenses.length) {
    setStorageStatus("There is no expense data to export yet.");
    return;
  }

  const rows = [
    ["Date", "Month", "Category", "Merchant", "Note", "Amount", "Currency"],
    ...state.expenses
      .slice()
      .sort((left, right) => left.date.localeCompare(right.date) || left.amount - right.amount)
      .map((expense) => [
        expense.date,
        getMonthKeyFromDate(expense.date),
        expense.category,
        expense.merchant,
        expense.note,
  ...
  link.href = url;
  link.download = `expense-atlas-${dateStamp}.csv`;
  document.body.appendChild(link);
  link.click();
  link.remove();
  window.URL.revokeObjectURL(url);

  setStorageStatus("Expense data exported as CSV.");
}
